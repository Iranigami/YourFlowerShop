...
class WriteoffActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        var writeoffview = setContentView(R.layout.activity_writeoff)
        supportActionBar?.setDisplayHomeAsUpEnabled(true)
        val linearLayout = findViewById<LinearLayout>(R.id.prod_layout)

        val addButton = findViewById<Button>(R.id.add_button)
        addButton.setOnClickListener{
            var bottomFragment = WriteoffBottomFragment();
            bottomFragment.show(supportFragmentManager,"TAG")
        }
        val getDataTask = DatabaseTask(this, linearLayout, writeoffview)
        getDataTask.execute()
    ...
    class DatabaseTask(private val mContext: Context, private val lLayout: LinearLayout, private val view: Unit) : AsyncTask<Void, Void, String>() {
        ...
        override fun doInBackground(vararg params: Void): String {
            ...
            try {
                ...
                val resultSet = db.executeQuery("SELECT * FROM writeoffs")
