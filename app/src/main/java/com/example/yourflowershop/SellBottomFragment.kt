...
class SellBottomFragment():BottomSheetDialogFragment() {
    override fun onCreateView(
        ...
        saveButton.setOnClickListener{
            if (edProduct.text.trim().toString().isNotEmpty() && edPrice.text.trim().toString().isNotEmpty() && edAmount.text.trim().toString().isNotEmpty()){
                ...
                val getDataTask = DatabaseTask(product, price, amount, con)
                getDataTask.execute()
...
class DatabaseTask(private val product: String, private val price: String, private val amount: String, private val mContext: Context) : AsyncTask<Void, Void, String>() {
    ...
    override fun doInBackground(vararg params: Void): String {
        ...
        try {
            ...
            if (!(dbProduct.next()))
            {
                ...
                val rs = db.executeQuery("SELECT COUNT(*) FROM sells")
                if (rs.next()) {
                    var count = rs.getInt(1) + 1
                    db.executeUpdate("INSERT INTO `sells` (`sell_id`, `price`, `product`, `user`, `date`, `amount`) VALUES ('$count', '$price', '$product', 'Дария', '$formattedDate','$amount');")
